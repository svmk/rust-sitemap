...
fn test_read_sitemap_check_urls() {
	...
	/* no 0 */
	assert_eq!(
		urls[0].loc.get_url().unwrap(),
		Url::parse("http://www.example.com/").unwrap()
	);
	assert_eq!(
		urls[0].lastmod.get_time().unwrap(),
	...
	assert_eq!(
		urls[0].priority.get_priority().unwrap(),
		0.8
	);

	/* no 1 */
	assert_eq!(
		urls[1].loc.get_url().unwrap(),
	...
	/* no 2 */
	assert_eq!(
		urls[2].loc.get_url().unwrap(),
		Url::parse("http://www.example.com/catalog?item=73&desc=vacation_new_zealand").unwrap()
	);
	assert_eq!(
		urls[2].lastmod.get_time().unwrap(),
	...
	/* no 3 */
	assert_eq!(
		urls[3].loc.get_url().unwrap(),
		Url::parse("http://www.example.com/catalog?item=74&desc=vacation_newfoundland").unwrap()
	);
	assert_eq!(
		urls[3].lastmod.get_time().unwrap(),
		DateTime::parse_from_rfc3339("2004-12-23T18:00:15+00:00").unwrap()
	);
	assert_eq!(
		urls[3].priority.get_priority().unwrap(),
		0.3
	);

	/* no 4 */
	assert_eq!(
		urls[4].loc.get_url().unwrap(),
		Url::parse("http://www.example.com/catalog?item=83&desc=vacation_usa").unwrap()
	);
	assert_eq!(
		urls[4].lastmod.get_time().unwrap(),
